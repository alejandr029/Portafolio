...
export default function CardProjects({project}: any) {
    return(
    <Link className="max-w-sm mx-1 border rounded-t-lg shadow bg-gray-800 border-gray-700 logo_shadow m-2 max-lg:mx-auto" to={`/Individual_project/${project.id}`}>
    ...
    <div className="p-5 text-left">
        ...
        <div className="flex items-center mt-2.5 mb-5 flex-wrap">
            {project.technologies.map((technology:any, index:any) => (
                <span key={index} className="bg-white text-black mt-2 text-xs font-semibold px-2.5 py-0.5 rounded ms-3"> {technology}</span>
            ))}
        </div>
